namespace CsCompiler.JsWriter.Expressions {
	...
	public class AssignmentExpression : IExpressionParser {
		public Expression Parse(CsExpression pStatement) {
			...
			if (((ex.lhs is CsElementAccess) || (ex.lhs is CsPrimaryExpressionMemberAccess)) && left.InternalType) {
				switch (ex.oper) {
					...
					case CsTokenType.tkMUL_EQ:
						...
						return new Expression(string.Format(left.Value, getter + JsHelpers.ConvertTokens(Helpers.GetTokenType(convertToken(ex.oper))) + right.Value), pStatement.entity_typeref);
				}
			}
			...
			return new Expression(string.Format("{0} {2} {1}", left.Value, right.Value, JsHelpers.ConvertTokens(Helpers.GetTokenType(ex.oper))), pStatement.entity_typeref);
		}
