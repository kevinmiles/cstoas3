	...
	public sealed class BlockParser {
		...
		private static void parseUsingStatement(CsStatement pArg1, CodeBuilder pSb) {
			...
			if (declaration == null) {
				...
				pSb.AppendFormat("var {0}:{1} = {2};",
					varname,
					JsHelpers.Convert(Helpers.GetType(e.Type)),
					e.Value
				);
				...
				sb.AppendFormat("var {0}:{1}",
					...
					JsHelpers.Convert(Helpers.GetType(declaration.type))
				);
		...
		private static void parseLocalVariable(CsStatement pStatement, CodeBuilder pSb) {
			...
			foreach (var declarator in localVariableDeclaration.declarators) {
				...
				sb.AppendFormat("var {0}:{1}",
					declarator.identifier.identifier,
					JsHelpers.Convert(Helpers.GetType(localVariableDeclaration.type))
				);
