namespace flash.Global {
	...
	[IsGeneric(true), NameAttribute("isFixed", "fixed", "")]
	public class Vector<T> : IEnumerable<T> {
		...
		public Vector(int pLength = int.MAX_VALUE, bool pIsFixed = true) {
			return;
		}
		...
		[Obsolete("Just for compatibility with C# compiler. DO NOT USE"), Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
		...
		/// <param name = "pCallback">The function to run on each item in the Vector. This function is invoked with three arguments: the current item from the Vector, the index of the item, and the Vector object.</param>
		/// <param name = "pThisObject">The object that the identifer this in the callback function refers to when the function is called.</param>
		public void forEach(Action<T, int, Vector<T>> pCallback, object pThisObject = null) {
			return;
		}
		...
		public int indexOf(T pSearchElement, int pFrom = 0) {
			return 0;
		}
		...
		/// <returns>A zero-based index position of the item in the Vector. If the searchElement argument is not found, the return value is -1.</returns>
		public int lastIndexOf(T pSearchElement, int pFrom = 0) {
			return 0;
		}
		...
		public Vector<T> map(Func<T, int, Vector<T>, T> pCallback, object pThisObject = null) {
			return null;
		}
		...
		public Vector<T> slice(int pStartIndex = 0, int pEndIndex = int.MAX_VALUE) {
			return null;
		}
		...
		public bool some(Func<T, int, Vector<T>, bool> pCallback, object pThisObject = null) {
			return false;
		}
		...
		sealed class VectorEnumerator : IEnumerator<T> {
